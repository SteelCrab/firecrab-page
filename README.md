...
`npm run build` is what Vercel runs: it builds the SPA, installs and builds the docs site,
then merges `docs-site/build` into `dist/` via `scripts/merge-docs-build.mjs`. The merge
must stay after `vite build` because Vite empties `dist/` first. The script refuses to run
if the docs build produced an `index.html`, which would overwrite the landing page.
